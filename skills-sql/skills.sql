...
FROM Models AS m JOIN Brands AS b ON brand_name = b.name 
WHERE brand_name = "Ford" AND m.name = "Mustang";
...
FROM Models AS m LEFT JOIN Brands AS b ON (brand_name = b.name) 
WHERE year = 1960;
...
FROM Brands AS b LEFT JOIN Models AS m ON brand_name = b.name 
WHERE b.discontinued IS NULL;
...
-- answer: An inner join only retrieves data that is present in both tables 
-- (where they overlap), but a left join returns all the data from the left 
-- table even if it is not present in the right table (it includes the entire
-- cirlce of the left table but only the overlapping part of the right table).
